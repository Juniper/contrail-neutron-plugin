...
class ContrailProviderDriver(driver_base.ProviderDriver):
    ...
    def update_resource(self, updated_opts, res_type, fq_name):
    ...
    def loadbalancer_update(self, old_lb, new_lb):
        ...
        """
        updated_opts = new_lb.to_dict(render_unsets=False)
        if self.is_updated(old_lb, updated_opts):
            fq_name = self.get_fq_name(old_lb.project_id, old_lb.loadbalancer_id)
            self.update_resource(updated_opts, c_const.LOADBALANCER_TYPE,
                                 fq_name)
    ...
    def listener_update(self, old_listener, new_listener):
        ...
        """
        try:
            updated_opts = new_listener.to_dict(render_unsets=False)
            if self.is_updated(old_listener, updated_opts):
                fq_name = self.get_fq_name(old_listener.project_id,
                                           old_listener.listener_id)
                self.update_resource(updated_opts, c_const.LISTENER_TYPE, fq_name)
    ...
    @check_opts(c_const.unsupported_pool_opts, c_const.CREATE_RES)
    def pool_create(self, pool):
        """Creates a new pool.
        """
        #TODO: set custom lb attributes supported by contrail
        try:
            o_listener = self._driver_lib.get_listener(pool.listener_id)
            project = self.read_project(o_listener.project_id)
    ...
    @check_opts(c_const.unsupported_pool_opts, c_const.UPDATE_RES)
    def pool_update(self, old_pool, new_pool):
        """Updates a pool.
        """
        try:
            updated_opts = new_pool.to_dict(render_unsets=False)
            if new_pool.session_persistence:
                updated_opts.update(new_pool.session_persistence)

            if self.is_updated(old_pool, updated_opts):
                fq_name = self.get_fq_name(old_pool.project_id,
                                           old_pool.pool_id)
                self.update_resource(updated_opts, c_const.POOL_TYPE, fq_name)
        ...
        status = {
            o_const.LOADBALANCERS: [{
                o_const.ID: old_pool.loadbalancer_id,
                o_const.PROVISIONING_STATUS: o_const.ACTIVE,
                o_const.OPERATING_STATUS: o_const.ONLINE
            }],
            o_const.LISTENERS: [{
                o_const.ID: old_pool.listener_id,
                o_const.PROVISIONING_STATUS: o_const.ACTIVE,
                o_const.OPERATING_STATUS: o_const.ONLINE
            }],
            o_const.POOLS: [{
                o_const.ID: old_pool.pool_id,
                o_const.PROVISIONING_STATUS: o_const.ACTIVE,
                o_const.OPERATING_STATUS: o_const.ONLINE
            }],
        }
    ...
    @check_opts(c_const.unsupported_member_opts, c_const.CREATE_RES)
    def member_create(self, member):
        """Creates a new member for a pool.
        """
        try:
            o_pool = self._driver_lib.get_pool(member.pool_id)
            project = self.read_project(o_pool.project_id)

            fq_name = list(project.fq_name)
            fq_name.append(member.pool_id)
            c_pool = self._api.loadbalancer_pool_read(fq_name=fq_name)

            id_perms = IdPermsType(enable=True,
                                   description="")

            contrail_member = LoadbalancerMember(name=member.member_id,
                                                 parent_obj=c_pool,
            ...
            self.set_properties(contrail_member,
                                c_const.MEMBER_TYPE,
                                LoadbalancerMemberType(),
                                member.to_dict())

            self._api.loadbalancer_member_create(contrail_member)

            status = {
                o_const.LOADBALANCERS: [{
                    o_const.ID: o_pool.loadbalancer_id,
                    o_const.PROVISIONING_STATUS: o_const.ACTIVE,
                    o_const.OPERATING_STATUS: o_const.ONLINE
                }],
                o_const.LISTENERS: [{
                    o_const.ID: o_pool.listener_id,
                    o_const.PROVISIONING_STATUS: o_const.ACTIVE,
                    o_const.OPERATING_STATUS: o_const.ONLINE
                }],
                o_const.POOLS: [{
                    o_const.ID: o_pool.pool_id,
                    o_const.PROVISIONING_STATUS: o_const.ACTIVE,
                    o_const.OPERATING_STATUS: o_const.ONLINE
                }],
                o_const.MEMBERS: [{
                    o_const.ID: member.member_id,
                    o_const.PROVISIONING_STATUS: o_const.ACTIVE,
                    o_const.OPERATING_STATUS: o_const.ONLINE
                }],
            }
    ...
    def member_delete(self, member):
        ...
        """
        try:
            fq_name = self.get_fq_name(member.project_id, member.pool_id)
            fq_name.append(member.member_id)
            res = self._api.loadbalancer_member_delete(fq_name=fq_name)
    ...
    @check_opts(c_const.unsupported_member_opts, c_const.UPDATE_RES)
    def member_update(self, old_member, new_member):
        """Updates a pool member.
        """
        try:
            updated_opts = new_member.to_dict(render_unsets=False)
            o_pool = self._driver_lib.get_pool(old_member.pool_id)

            if self.is_updated(old_member, updated_opts):
                fq_name = self.get_fq_name(o_pool.project_id, o_pool.pool_id)
                fq_name.append(old_member.member_id)
                self.update_resource(updated_opts, c_const.MEMBER_TYPE,
                                     fq_name)
            ...
            status = {
                ...
                o_const.MEMBERS: [{
                    o_const.ID: new_member.member_id,
                    o_const.PROVISIONING_STATUS: o_const.ACTIVE,
                    o_const.OPERATING_STATUS: o_const.ONLINE
                }],
            }
