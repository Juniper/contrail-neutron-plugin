...
class BaremetalVIF(object):
    ...
    def _get_vnc_conn(cls):
        ...
        if vnc_conn:
            return vnc_conn
        # Retry till the api-server is up
        while True:
            try:
    ...
    @classmethod
    def is_port_baremetal(cls, port):
    ...
    @classmethod
    def get_physical_port_info(cls, port):
    ...
    def bind_baremetal_port(cls, port):
        ...
        if not vnc_conn.physical_router_read(
            ...
            LOG.debug("Binding skipped: physical router is not "
                      "registered.")
            return
        cls._get_or_create_physical_interface(piface_fq_name)
        liface = cls._get_or_create_logical_interface(liface_fq_name)
    ...
    @classmethod
    def _get_or_create_physical_interface(cls, piface_fq_name):
        piface = None
        try:
            piface = cls._get_vnc_conn().physical_interface_read(
        ...
        if not piface:
            iface = vnc_api.PhysicalInterface(parent_type='physical-router',
                                              fq_name=piface_fq_name)
            piface = cls._get_vnc_conn().physical_interface_create(iface)

    @classmethod
    def _get_or_create_logical_interface(cls, liface_fq_name):
        liface = None
        try:
            liface = cls._get_vnc_conn().logical_interface_read(
        ...
        if not liface:
            iface = vnc_api.LogicalInterface(parent_type='physical-interface',
                                             fq_name=liface_fq_name)
            iface_uuid = cls._get_vnc_conn().logical_interface_create(iface)
            liface = cls._get_vnc_conn().logical_interface_read(id=iface_uuid)

        return liface

    @classmethod
    def unbind_baremetal_port(cls, port):
        vnc_conn = cls._get_vnc_conn()
