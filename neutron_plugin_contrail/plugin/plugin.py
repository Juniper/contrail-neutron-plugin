...
import requests
from vnc_api.utils import getCertKeyCaBundle

from neutron_plugin_contrail.common.exceptions import VncApiServersUnavailable
from neutron_plugin_contrail.plugin import plugin_base
...
class ContrailPlugin(plugin_base.ContrailPluginBase):
    ...
    def _build_auth_details(self):
        ...
        if self._apiusessl and apicafile:
            ...
            self._apicertbundle = getCertKeyCaBundle(_DEFAULT_API_CERT_BUNDLE,
                                                     certs)
    ...
    def _relay_request(self, url_path, data=None):
        ...
        api_server_count = len(self.api_servers)
        ...
        LOG.critical(msg)
        raise VncApiServersUnavailable(
            api_servers=', '.join(cfg.CONF.APISERVER.api_server_ip.split()))
    ...
    def remove_router_interface(self, context, router_id, interface_info):
        ...
        status_code, res_info = self._request_backend(context, res_dict,
                                                      'router', 'DELINTERFACE')
        if status_code != requests.codes.ok:
            plugin_base._raise_contrail_error(
