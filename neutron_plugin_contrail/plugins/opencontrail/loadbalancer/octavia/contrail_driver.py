...
class ContrailProviderDriver(driver_base.ProviderDriver):
    ...
    def listener_create(self, listener):
        ...
        try:
            ...
            octavia_lb = self._octavia_driver_lib.get_loadbalancer(listener.loadbalancer_id)
            project = self.read_project(octavia_lb.project_id)


            if not self.is_name_unique('loadbalancer-listener', project, listener.listener_id):
                self.raise_error("Listener with uuid %s already exists" % listener.listener_id)


            if listener.admin_state_up == Unset:
                listener.admin_state_up = True

            id_perms = IdPermsType(enable=True, description=listener.description)

            contrail_listener = LoadbalancerListener(name=listener.listener_id,
                                                     parent_obj=project,
            ...
            if listener.default_tls_container_ref:
                props.set_default_tls_container(listener.default_tls_container_ref)
            ...
            status = {'listeners': [{"id": listener.listener_id,
                                     "provisioning_status": "ACTIVE",
                                     "operating_status": "ONLINE"}],
	...
	"""
        ...
        status = {'listeners': [{"id": new_listener.listener_id,
                                 "provisioning_status": "ACTIVE",
                                 "operating_status": "ONLINE"}],
                  'loadbalancers': [{"id": old_listener.loadbalancer_id,
                                     "provisioning_status": "ACTIVE",
                                     "operating_status": "ONLINE"}]}
        self.update_status(status)
