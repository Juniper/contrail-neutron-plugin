...
from vnc_api.vnc_api import HttpError
...
class LoadbalancerPoolManager(ResourceManager):
    ...
    def update_properties(self, pool_db, id, p):
        ...
        if 'custom_attributes' in p:
            custom_attributes =\
                pool_db.get_loadbalancer_pool_custom_attributes()
            ...
            if self.create_update_custom_attributes(p['custom_attributes'],
                                                    custom_attributes):
                pool_db.set_loadbalancer_pool_custom_attributes(
                    custom_attributes)
                change = True
