...
from vnc_api.vnc_api import Loadbalancer
from vnc_api.vnc_api import LoadbalancerType
from vnc_api.vnc_api import NoIdError
from vnc_api.vnc_api import SecurityGroup
from vnc_api.vnc_api import VirtualMachineInterface

from neutron_plugin_contrail.services.loadbalancer import utils
from neutron_plugin_contrail.services.loadbalancer.resource_manager import\
    EntityInUse
from neutron_plugin_contrail.services.loadbalancer.resource_manager import\
    ResourceManager
...
class LoadbalancerManager(ResourceManager):
    ...
    def create(self, context, loadbalancer):
        ...
        lb_neutron = loadbalancer['loadbalancer']
        if (lb_neutron['provider'] == ATTR_NOT_SPECIFIED):
            lb_neutron['provider'] = "opencontrail"
        sas_obj = self.check_provider_exists(lb_neutron['provider'])
        tenant_id = self._get_tenant_id_for_create(context, lb_neutron)
        ...
        name = self._get_resource_name('loadbalancer', project,
                                       lb_neutron['name'], obj_uuid)
        id_perms = IdPermsType(
            enable=True, description=lb_neutron['description'])
        lb = Loadbalancer(name, project, uuid=obj_uuid,
                          loadbalancer_provider=lb_neutron['provider'],
                          id_perms=id_perms, display_name=lb_neutron['name'])
        lb.set_service_appliance_set(sas_obj)

        vmi, vip_address, vip_subnet_id = self._create_virtual_interface(
            project,
            obj_uuid,
            lb_neutron['vip_subnet_id'],
            lb_neutron['vip_network_id'],
            lb_neutron['vip_address'])

        lb.set_virtual_machine_interface(vmi)

        lb_neutron['provisioning_status'] = 'ACTIVE'
        lb_neutron['operating_status'] = 'ONLINE'
        lb_neutron['vip_subnet_id'] = vip_subnet_id
        props = self.make_properties(lb_neutron)
    ...
    def _update_loadbalancer_properties(self, props, id, lb):
        ...
        for field in immutable:
            ...
            if getattr(props, field) != lb[field]:
                msg = ("Attribute %s in loadbalancer %s is immutable" %
                       (field, id))
                raise BadRequest(resource='loadbalancer', msg=msg)
