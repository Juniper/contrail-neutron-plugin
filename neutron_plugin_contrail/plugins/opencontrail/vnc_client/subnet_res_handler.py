...
class SubnetMixin(object):
    ...
    def _validate_and_get_subnetpool_obj(self, subnetpool_id, vn_obj):
        ...
        sp_refs = vn_obj.get_subnet_pool_refs()
        for sp_ref in sp_refs or []:
            if sp_ref['uuid'] != subnetpool_id:
                self._raise_contrail_exception(
...
class SubnetCreateHandler(res_handler.ResourceCreateHandler, SubnetMixin):
    ...
    def resource_create(self, context, subnet_q):
        ...
        if subnetpool_id:
            sp_obj = self._validate_and_get_subnetpool_obj(subnetpool_id,
                                                           vn_obj)
