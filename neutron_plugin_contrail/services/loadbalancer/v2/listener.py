...
from vnc_api.vnc_api import LoadbalancerListener
from vnc_api.vnc_api import LoadbalancerListenerType
...
class ListenerManager(ResourceManager):
    ...
    def create(self, context, listener):
        ...
        if listener['loadbalancer_id']:
            try:
                lb = self._api.loadbalancer_read(
                    id=listener['loadbalancer_id'])
            except NoIdError:
                raise loadbalancerv2.EntityNotFound(
                    name='Loadbalancer', id=listener['loadbalancer_id'])
        ...
        name = self._get_resource_name('loadbalancer-listener',
                                       project, listener['name'], obj_uuid)
        ...
        props = self.make_properties(listener)
    ...
    def delete_listener(self, context, id):
        try:
            self._api.loadbalancer_listener_read(id=id)
        except NoIdError:
